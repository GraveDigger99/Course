 ...
 */
    ...
    (() => {
        ...
        function initSliders() {
            ...
            if (document.querySelector(".block-other-slider")) new core(".block-other-slider", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 2,
                ...
                loop: true,
                navigation: {
                    prevEl: ".controll__prev-other-slide",
                    nextEl: ".controll__next-other-slide"
                },
                breakpoints: {
                    320: {
                        slidesPerView: 1
            ...
            if (document.querySelector(".events-slider__slider")) new core(".events-slider__slider", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                spaceBetween: 30,
...
*/
        ...
        const dataItemsView = document.querySelector("[data-items-view]");
        if (dataItemsView) {
            const itemsView = () => {
                document.addEventListener("click", (e => {
                    let targetElement = e.target;
                    if (targetElement.closest("[data-view-grid]")) {
                        document.documentElement.querySelector("[data-row]").classList.remove("active");
                        document.documentElement.classList.add("grid-view");
                        targetElement.classList.add("active");
                    } else if (targetElement.closest("[data-row]")) {
                        document.documentElement.classList.remove("grid-view");
                        document.documentElement.querySelector("[data-view-grid]").classList.remove("active");
                    }
                }));
            };
            itemsView();
        ...
        if (dataTimer) {
            const discountDate = new Date("Jan 1 2024 00:00:00");
        ...
        const filterItemsWrapper = document.querySelector(".cards-courses");
        if (filterItemsWrapper) {
            function filterItemsCountToButton() {
                if (filterItemsWrapper) {
                    var filterItems = filterItemsWrapper.querySelectorAll("[data-filter]");
                    const filterİtemsArray = Array.from(filterItems);
                    const filterItemsDataArrayMarketing = filterİtemsArray.filter((el => "marketing" === el.dataset.filter));
                    const filterItemsDataArrayManagement = filterİtemsArray.filter((el => "management" === el.dataset.filter));
                    const filterItemsDataArrayHr = filterİtemsArray.filter((el => "hr" === el.dataset.filter));
                    const filterItemsDataArrayDesign = filterİtemsArray.filter((el => "design" === el.dataset.filter));
                    const filterItemsDataArrayDevelopment = filterİtemsArray.filter((el => "development" === el.dataset.filter));
                    const filterEnjoy = document.querySelector(".filter-enjoy");
                    if (filterEnjoy) {
                        const filterButtons = filterEnjoy.querySelectorAll("[data-filter]");
                        filterButtons.forEach((filterButton => {
                            if ("all" === filterButton.dataset.filter) filterButton.querySelector("small").innerHTML = filterItems.length;
                            if ("marketing" === filterButton.dataset.filter) filterButton.querySelector("small").innerHTML = filterItemsDataArrayMarketing.length;
                            if ("management" === filterButton.dataset.filter) filterButton.querySelector("small").innerHTML = filterItemsDataArrayManagement.length;
                            if ("hr" === filterButton.dataset.filter) filterButton.querySelector("small").innerHTML = filterItemsDataArrayHr.length;
                            if ("design" === filterButton.dataset.filter) filterButton.querySelector("small").innerHTML = filterItemsDataArrayDesign.length;
                            if ("development" === filterButton.dataset.filter) filterButton.querySelector("small").innerHTML = filterItemsDataArrayDevelopment.length;
                        }));
                    }
                }
            }
            filterItemsCountToButton();
        }
        if (filterItemsWrapper) {
            function filter(e) {
                const filtersButtonsWrapper = document.querySelector(".filter-enjoy");
                const filterItems = filterItemsWrapper.querySelectorAll("[data-filter]");
                if (filtersButtonsWrapper) {
                    const filtersButtonShowAll = filtersButtonsWrapper.querySelector("[data-filter]");
                    let targetElement = e.target;
                    const targetElementFilterValue = targetElement.dataset.filter;
                    if (targetElement.closest("[data-filter]")) {
                        document.querySelector(".cards-courses_column").classList.toggle("filter-active");
                        const activeButton = document.querySelector("._active-button");
                        if (activeButton && activeButton !== targetElement) activeButton.classList.remove("_active-button");
                        if (filterItems.length) filterItems.forEach((filterItem => {
                            filterItem.classList.remove("hide");
                            if (filterItem.dataset.filter !== targetElementFilterValue && targetElement !== filtersButtonShowAll) filterItem.classList.add("hide");
                        }));
                        targetElement.classList.toggle("_active-button");
                    }
                }
            }
